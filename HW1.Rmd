...
```{r}
arr_delay_by_airline <- data.frame(key=names(arr_delay_by_airline), value=arr_delay_by_airline)


highest_avg <- head(arr_delay_by_airline, 1)[1,1]
lowest_avg <- tail(arr_delay_by_airline, 1)[1, 1]

highest_avg
lowest_avg
```
```{r}
negative_delay_times <- arr_delay_by_airline[arr_delay_by_airline[2] < 0, ]
negative_delay_times
```

The airline with the highest average delay is `r highest_avg`. The airline with the lowest average delay is `r lowest_avg`. The two airlines with negative average delay times are `r negative_delay_times[1,1]` and `r negative_delay_times[2,1]`.
